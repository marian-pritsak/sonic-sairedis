...
PfcWatchdog::PfcCounterIds::PfcCounterIds(
    ...
    portId(port), portCounterIds(portIds), queueCounterIds(queueIds)
{
}
        ...
        _In_ const std::vector<sai_port_stat_t> &counterIds)
{
    ...
    std::lock_guard<std::mutex> lock(wd.m_mutex);

    sai_object_id_t portId = queueIdToPortId(queue);
    if (portId == SAI_NULL_OBJECT_ID)
    {
        return;
    }

    auto it = wd.m_counterIdsMap.find(queue);
    ...
    auto pfcCounterIds = std::shared_ptr<PfcCounterIds>(new PfcCounterIds(portId, counterIds, {}));
    wd.m_counterIdsMap.emplace(queue, pfcCounterIds);
        ...
        _In_ sai_object_id_t queue)
{
    ...
    if (it == wd.m_counterIdsMap.end())
    {
        SWSS_LOG_ERROR("Trying to remove nonexisting queue counter Ids 0x%lx", queue);
        return;
    }
        ...
        _In_ swss::Table &countersTable)
{
    ...
    for (const auto &kv: m_counterIdsMap)
    {
        ...
        if (status != SAI_STATUS_SUCCESS)
        {
            SWSS_LOG_ERROR("Failed to get stats of queue 0x%lx: %d", queueId, status);
            continue;
        }
        ...
        countersTable.set(sai_serialize_object_id(queueId), values, "");
    }
}
