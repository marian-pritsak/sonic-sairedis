...
void saiLoglevelNotify(std::string apiStr, std::string prioStr)
{
    static const std::map<std::string, sai_log_level_t> saiLoglevelMap = {
        { "EMERG", SAI_LOG_CRITICAL },
        { "ALERT", SAI_LOG_CRITICAL },
        { "CRIT", SAI_LOG_CRITICAL },
        { "ERROR", SAI_LOG_ERROR },
        { "WARN", SAI_LOG_WARN },
        { "NOTICE", SAI_LOG_NOTICE },
        { "INFO", SAI_LOG_INFO },
        { "DEBUG", SAI_LOG_DEBUG }
    };
...
int main(int argc, char **argv)
{
    ...
    swss::Logger::PriorityChangeObserver saiObservers;
    for (const auto& i : saiApiMap)
    {
        saiObservers[i.first] = saiLoglevelNotify;
    }

    swss::Logger::linkToDb("syncd", &saiObservers);
